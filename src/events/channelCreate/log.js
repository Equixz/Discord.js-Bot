...
module.exports = async (client, channel) => {
    try {
        channel.guild.fetchAuditLogs({ type: AuditLogEvent.ChannelCreate, }) .then(async audit => {
            ...
            .addFields(
                { name: 'Channel Name', value: `${name} (<#${id}>)`, inline: true },
                { name: 'Channel Type', value: `${type}`, inline: true },
                { name: 'Channel ID', value: `${id}`, inline: false },
                { name: 'Created By', value: `${executor.tag} (<@${executor.id}>)`, inline: true }
            )
            .setTimestamp()
        ...
        console.log(`There was an error when creating a create channel log: ${error}`)
    }
};